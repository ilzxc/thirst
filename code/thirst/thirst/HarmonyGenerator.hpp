...
{
  public:
    HarmonyGenerator( std::vector< std::pair< float, float > > source,
                      int numSolutions = 1, float mutationRate = 0.001f,
  ...
  private:
    ...
    int scale( float input, float in_min, float in_high, float out_min,
               float out_high );
