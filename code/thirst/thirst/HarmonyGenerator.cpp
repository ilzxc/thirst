...
HarmonyGenerator::HarmonyGenerator( vector< pair< float, float > > source, int numSolutions, float mutationRate, int numGenerators )
...
void HarmonyGenerator::setup()
{
    ...
    for ( auto i = 0; i < numGenerators; ++i ) {
        armonie.push_back( make_shared< GenHarmony >( numPartials, lowFreq, highFreq, true ) );
    }
}
...
void HarmonyGenerator::update()
{
    ...
    //cout << "Current population average roughness : " << sum / armonie.size() << " / " << targetRoughness << endl;
...
vector< int > HarmonyGenerator::scaledFitnesses( const vector< float >& fitnesses )
{
    vector< int > result;
    result.reserve( armonie.size() );
...
int HarmonyGenerator::scale( float input, float in_low, float in_high, float out_low, float out_high )
{
    auto in_range = in_high - in_low;
    if ( in_range == 0 ) return rand.randInt( 30 );
