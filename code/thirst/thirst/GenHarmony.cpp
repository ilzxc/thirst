...
std::ostream& operator<<( std::ostream& lhs, const GenHarmony& rhs )
{
    ...
    for ( const auto& partial : rhs.aggregate ) {
        result += " " + to_string( partial.frequency ) + " " + to_string( partial.amplitude );
    }
    result += ";";
    lhs << result;
    return lhs;
...
shared_ptr< GenHarmony > GenHarmony::crossover( const std::shared_ptr< GenHarmony > partner )
{
    ...
    for ( auto i = 0; i < midpoint; ++i ) child->aggregate[ i ] = aggregate[ i ];
    for ( auto i = midpoint; i < aggregate.size(); ++i ) child->aggregate[ i ] = partner->aggregate[ i ];
    return child;
}
